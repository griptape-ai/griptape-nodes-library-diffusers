...
### Rule 9 — Every new pipeline type needs a memory-estimation family entry

Adding a new pipeline TYPE always requires a matching entry in [`memory_estimation/family_registry.py`](../../../modular_diffusion_nodes_library/memory_estimation/family_registry.py) `_FAMILY_BY_PIPELINE_NAME`, keyed by the exact same pipeline-class-name string used in `_DRIVER_REGISTRY`. Skipping this entry does not break at import time; it silently degrades the new pipeline's memory estimate to weights-only.

Load [`./references/memory-estimation.md`](./references/memory-estimation.md) before filling in Phase B's memory-estimation section or doing Phase C step 5's registration edits — it has the `MemoryFamily` decision criteria, the denoiser-config-adapter reuse-vs-new rule, and the verification commands.
...
2. Read [`driver_factory.py`](../../../modular_diffusion_nodes_library/latent_pipeline_drivers/driver_factory.py), [`parameters/pipelinetype_parameters.py`](../../../modular_diffusion_nodes_library/parameters/pipelinetype_parameters.py), [`parameters/pipeline_parameters.py`](../../../modular_diffusion_nodes_library/parameters/pipeline_parameters.py), and [`memory_estimation/family_registry.py`](../../../modular_diffusion_nodes_library/memory_estimation/family_registry.py) to understand the four registries the new pipeline class name must appear in (see Rule 9 for the family-registry entry).
...
Load [`./references/plan-template.md`](./references/plan-template.md) and produce a plan matching its structure exactly — every section filled in, none omitted. It covers closest precedent, the pipeline class, the modular-vs-`DiffusionPipeline` method split, Provider classification, runtime-parameters reuse, files to create/modify, the memory-estimation family (see Rule 9 / [`./references/memory-estimation.md`](./references/memory-estimation.md)), latent contract compliance, the variants checklist, runtime parameter defaults/tooltips (Rule 4), and open questions.
...
5. **Registration edits**:
   - Always: one entry in [`driver_factory.py`](../../../modular_diffusion_nodes_library/latent_pipeline_drivers/driver_factory.py) `_DRIVER_REGISTRY`; one or more `case` clauses in [`pipeline_parameters.py`](../../../modular_diffusion_nodes_library/parameters/pipeline_parameters.py) `set_runtime_parameters`; one entry in [`memory_estimation/family_registry.py`](../../../modular_diffusion_nodes_library/memory_estimation/family_registry.py) `_FAMILY_BY_PIPELINE_NAME`, plus (unless the family is UNET_SDPA) one entry — new or reused — in `_DENOISER_FIELD_ADAPTERS` (Rule 9).
   ...
   - Unless UNET_SDPA: `uv run python -c "from modular_diffusion_nodes_library.memory_estimation.family_registry import _DENOISER_FIELD_ADAPTERS; assert '<PipelineClassName>' in _DENOISER_FIELD_ADAPTERS"` and manually confirm the adapter's attribute reads match the new pipeline's actual transformer config (Rule 9).
   ...
   **Surgical-edits check**: diff each shared file — including `family_registry.py` — and confirm only added lines / one inserted block are present — no reformatted neighbours, no reordered entries, no touched sibling cases.
...
- [`./references/plan-template.md`](./references/plan-template.md) — the Phase B plan template.
- [`./references/memory-estimation.md`](./references/memory-estimation.md) — Rule 9's `MemoryFamily` / denoiser-config-adapter selection guide.
