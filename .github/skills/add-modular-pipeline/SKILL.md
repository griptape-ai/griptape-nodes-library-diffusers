...
- **`encode_media` for video drivers**: Image-only drivers raise `NotImplementedError` for `VideoMedia`; video-only (T2V) drivers raise for `ImageMedia`. "Vice versa" does NOT mean "video drivers raise for all media" — T2V drivers MUST implement `encode_media` for `VideoMedia` when the pipeline supports video-to-video (V2V). I2V drivers SHOULD additionally implement `encode_media` for `ImageMedia` to enable latent composition workflows. Check the Variants checklist (V2V item) before deciding.
- **`add_noise_to_latent` for video drivers**: This method is NOT image-only. V2V workflows (encoding a source video, adding noise at a given `strength`, then denoising) require a working `add_noise_to_latent` on video drivers. For flow-matching schedulers (`FlowMatchEulerDiscreteScheduler`) use `scheduler.scale_noise`; for DDPM-family schedulers use `scheduler.add_noise`. Only raise `NotImplementedError` when you have verified the underlying pipeline has no V2V support and have cited the diffusers source proving it.
