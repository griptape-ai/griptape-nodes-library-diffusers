...
3. **Single-file loader metadata** → if the variant pipeline class is a fresh entry in `_DRIVER_REGISTRY` and users may load its transformer from a single `.gguf`/`.safetensors` file, add the relevant `model_type` → variant-pipeline-class-name entries to [`component_loading/pipeline_type_registry.py`](../../../modular_diffusion_nodes_library/component_loading/pipeline_type_registry.py) `MODEL_TYPE_TO_PIPELINE_TYPE`, and drop the canonical transformer config into `modular_diffusion_nodes_library/component_loading/bundled_configs/<model_type>/transformer/config.json`. Skip this step for pure runtime pipe-swaps that share the base pipeline's transformer weights.
