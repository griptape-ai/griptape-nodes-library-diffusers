...
- `_get_spatial_alignment()` — override in video drivers to return the pixel-space divisor for height and width. Typically `pipe.vae_scale_factor_spatial * pipe.transformer.config.patch_size[1]`. Return `None` for image-only drivers. Called by the base `validate_dimensions` and `align_dimensions`.
