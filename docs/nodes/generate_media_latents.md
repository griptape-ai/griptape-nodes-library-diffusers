...
| LTX2 | The base, video-conditioning, and IC-LoRA pipelines generate video and audio jointly in one denoising loop. The output latent carries the audio in its metadata, which is muxed by [Decode Media Latent](decode_media_latent.md). `reference_conditions` accepts `Media Gen Conditioning`, `ImageUrlArtifact`, and `VideoUrlArtifact`; images are passed to Diffusers as still-image references. The HDR IC-LoRA path does not produce audio. |
...
| WAN VACE | Adds `source_media`, `mask`, and `reference_images` conditioning inputs, each fed from a [Media Gen Conditioning](media_gen_conditioning.md) node (or a direct image/video connection). `mask` and `reference_images` require `source_media` to be connected — otherwise the run fails validation. If `mask` is omitted, it's auto-derived from `source_media` (black/preserve where the source has content, white/generate elsewhere). `reference_images` supports at most 8 images. |
