...
| LTX2 | the base, video-conditioning, and IC-LoRA pipelines generate video and audio jointly in one denoising loop. The output latent carries the audio in its metadata, which is muxed by [Decode Media Latent](decode_media_latent.md). The HDR IC-LoRA path does not produce audio. |
