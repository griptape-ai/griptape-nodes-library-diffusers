...
**Writes a `LatentArtifact` to disk as a torch `.pt` file. Useful for caching expensive denoising results, debugging, or building offline test fixtures.**
...
- **`.pt` files are large.** A 1024×1024 SDXL latent is ~4 MB; an LTX video latent can be hundreds of MB. Don't commit these to source control.
